export class Event {
  ...
  public constructor(data: any) {
    if (data) {
      if ( data.images ) { this.image = data.images[0].url; }
      if ( data.name ) { this.title = data.name; }
      if ( data._embedded.venues ) { this.venues = `${data._embedded.venues[0].name}in${data._embedded.venues[0].city.name}`; }
      if ( data.info ) { this.description = data.info; }
